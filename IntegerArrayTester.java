...
final class IntegerArray {
    ...
    public IntegerArray getSorted() {
        ...
        Arrays.sort(arr1);
        IntegerArray sorted = new IntegerArray(arr1);
        return sorted;
    }
    public int sum() {
    ...
    public IntegerArray concat(IntegerArray ia) {
        IntegerArray a = ia.getSorted();
        IntegerArray b = this.getSorted();
        int len = ia.length() + this.length();
        int[] ar = new int[ia.length()];
        int[] ar1 = new int[this.length()];

        for (int i = 0; i < ia.length(); i++) {
            ar[i] = ia.getElementAt(i);
        }
        for (int i = 0; i < this.length(); i++) {
            ar1[i] = this.getElementAt(i);
        }

        int[] ar2 = IntStream.concat(Arrays.stream(ar), Arrays.stream(ar1)).toArray();
		Arrays.sort(ar2);
        IntegerArray joined = new IntegerArray(ar2);
        return joined;
    }
...
if (this == o) return true;
        ...
        IntegerArray ia = (IntegerArray) o;
        if (this.length() != ia.length()) return false;
        for (int i = 0; i < this.length(); i++) {
        	if (this.getElementAt(i) != ia.getElementAt(i)) return false;
        }
        return true;
    }
